...
"""
import math  # импорт библиотеки math, из которой будем использовать метод возвращающий значение числа пи
...
print('=' * 19, 'Решение через именные функции', '=' * 20)
...
print()
print('=' * 5, 'Решение с анонимными функциями и функциями высшего порядка', '=' * 5)
num_pi = math.pi  # число пи
sq_orbits2 = list(map(lambda el: el[0] * el[1] * num_pi, orbits))  # определяем значения площадей орбит планет
...
print()
print('=' * 18, 'Решение с использованием словаря', '=' * 18)
sq_orbits2 = list(map(lambda el: el[0] * el[1] * num_pi, orbits))  # определяем значения площадей орбит планет
sq_orbits = list(map(lambda every: round(every, 7), sq_orbits2))  # округляем float до 7 знаков
biggest_square = max(sq_orbits)  # находим максимальное значение в списке площадей
# формируем словарь, значения ключей - площади орбит, значения элементов - кортежи с орбитами планет
res_dict = dict(zip(sq_orbits, orbits))
print('Площади орбит:', list(res_dict.keys()))  # здесь выводим ключи словаря
# далее выводим значение по ключю, т.е. ключ равный biggest_square, выведет сопоставленное с ним значение
# для получения ключа использован метод get(), т.к. он не выдает ошибки в случае отсутствия ключа в словаре
print('Орбита самой далекой планеты:', *res_dict.get(biggest_square))
print('=' * 70)
